...
public class ManejadorTelefonosIp implements ICRUD {
    ...
    private static void mezclaDesc(ArrayList<TelefonosIp> array, TelefonosIp temp[], int izq, int fizq, int der, int fder) {
        int postemp = izq, numele, i;
        numele = fder - izq + 1;
        while (izq <= fizq && der <= fder) {
            if (array.get(izq).getNombre().compareTo(array.get(der).getNombre()) > 0) {
    ...
    private static void ordenarMDesc(ArrayList<TelefonosIp> array, TelefonosIp temp[], int izq, int der) {
        int centro;
        if (izq < der) {
            centro = (int) (izq + der) / 2;
    ...
    public void ordenarMezclaDesc() {
        TelefonosIp temp[] = new TelefonosIp[cateofi.arreglotelefonosip.size()];
        ordenarMDesc(cateofi.arreglotelefonosip, temp, 0, cateofi.arreglotelefonosip.size() - 1);
    }
    ...
    public static int pivoteDesc(ArrayList<TelefonosIp> array, int prim, int ult, int piv) {
        float p = array.get(piv).getPrecio();
        int j = prim;
        int i;
        intercambio(array, piv, ult);
    ...
    public static void quicksorttDesc(ArrayList<TelefonosIp> array, int inicio, int fin) {
        int medio;
        if (inicio < fin) {
